    ...
    override fun collectTransform(boneName: String): TransformContext<A10Entity>? {
        return when (boneName) {
            "root" -> TransformContext { bone, vehicle, _ ->
                bone.setHidden(
                    hideForTurretControllerWhileZooming && vehicle.getWeaponIndex(0) == 2
                )
            }
            ...
            "wingLB" -> TransformContext { bone, vehicle, state ->
                bone.setRotX(Mth.lerp(state.partialTick, vehicle.flap2LRotO, vehicle.flap2LRot) * Mth.DEG_TO_RAD)
            }

            "wingRB" -> TransformContext { bone, vehicle, state ->
                bone.setRotX(Mth.lerp(state.partialTick, vehicle.flap2RRotO, vehicle.flap2RRot) * Mth.DEG_TO_RAD)
            }
            ...
            "gear", "gear2", "gear3" -> TransformContext { bone, vehicle, state ->
                bone.setRotX(vehicle.gearRot(state.partialTick) * Mth.DEG_TO_RAD)
            }
