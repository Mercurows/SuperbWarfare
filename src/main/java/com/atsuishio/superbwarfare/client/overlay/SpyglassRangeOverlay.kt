...
@OnlyIn(Dist.CLIENT)
object SpyglassRangeOverlay : IGuiOverlay {
    const val ID: String = Mod.MODID + "_spyglass_range"
    ...
    override fun render(
        ...
        if (((player.isUsingItem && player.getUseItem()
            ...
            if (player.getUseItem().`is`(ModItems.ARTILLERY_INDICATOR.get())) {
                ...
                // 火炮位置
                val tags = stack.getOrCreateTag().getList(ArtilleryIndicator.TAG_CANNON, Tag.TAG_COMPOUND.toInt())
