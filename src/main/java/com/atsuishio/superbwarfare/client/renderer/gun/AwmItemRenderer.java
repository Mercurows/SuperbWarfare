...
public class AwmItemRenderer extends CustomGunRenderer<AwmItem> {
    ...
    @Override
    public void renderRecursively(PoseStack stack, AwmItem animatable, GeoBone bone, RenderType type, MultiBufferSource buffer, VertexConsumer bufferIn, boolean isReRender, float partialTick, int packedLightIn, int packedOverlayIn, float red,
            ...
            if (this.renderPerspective == ItemDisplayContext.FIRST_PERSON_RIGHT_HAND || this.renderPerspective == ItemDisplayContext.THIRD_PERSON_RIGHT_HAND) {
                ...
                if (this.renderPerspective == ItemDisplayContext.FIRST_PERSON_RIGHT_HAND) {
                    ...
                    if (GunData.from(itemStack).attachment.get(AttachmentType.SCOPE) == 2) {
                        if (!itemStack.getOrCreateTag().getBoolean("ScopeAlt") && (bone.getName().endsWith("_hide2") || bone.getName().equals("Barrel"))) {
                    ...
                    if (GunData.from(itemStack).attachment.get(AttachmentType.SCOPE) == 3 && (bone.getName().endsWith("_hide3") || bone.getName().equals("Barrel"))) {
                        bone.setHidden(ClientEventHandler.zoomPos > 0.7 && ClientEventHandler.zoom);
                    }
