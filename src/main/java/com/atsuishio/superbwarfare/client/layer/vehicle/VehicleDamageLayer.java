...
public class VehicleDamageLayer extends GeoRenderLayer {
  public VehicleDamageLayer(GeoRenderer entityRenderer) {
  ...
  public void render(PoseStack poseStack, GeoAnimatable animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
    if (animatable instanceof VehicleEntity vehicle) {
      ...
      getRenderer().reRender(bakedModel, poseStack, bufferSource, animatable, blackRender, bufferSource.getBuffer(blackRender), partialTick, packedLight, packedOverlay, color, color, color, 1.0F);
    }
  }
