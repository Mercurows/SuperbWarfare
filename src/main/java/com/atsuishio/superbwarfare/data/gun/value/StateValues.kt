...
/**
 * Value wrappers backed by [GunState] instead of a [CompoundTag].
 *
 ...
 * [GunState]. Reads go through the state snapshot and writes go through [GunData.update], which writes
 * the scalars straight into the gun tag on the stack — so a caller can no longer forget to `save()`.
 *
 * The backing tag of the superclass is a shared dummy: every accessor that matters is overridden here.
 */
