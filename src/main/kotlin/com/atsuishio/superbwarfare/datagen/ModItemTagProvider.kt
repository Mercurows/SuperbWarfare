...
class ModItemTagProvider(
     ...
     * 新增一个配件物品时把 `RegistryObject` 追加到对应槽位的列表里即可，新增槽位则再加一个键。
     */
    private fun attachmentItemsBySlot(): Map<AttachmentType, List<DeferredHolder<Item, out Item>>> = mapOf(
    ...
    private fun addAttachmentItem(item: DeferredHolder<Item, out Item>, slot: AttachmentType) {
        val rarity = item.get().defaultInstance.getOrDefault(DataComponents.RARITY, Rarity.COMMON)
        val rarityTag = ModTags.Items.attachmentRarityTag(slot, rarity)
            ?: error("Unsupported attachment rarity for ${item.id}: $rarity")

        this.tag(rarityTag).add(item.get())
    }
}
