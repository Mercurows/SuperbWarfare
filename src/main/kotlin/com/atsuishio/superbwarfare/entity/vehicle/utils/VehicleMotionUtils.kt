            ...
            for (obb in obbs) {
                ...
                for (aabb in allAabbs) {
                    ...
                    if (movDotMtv < 0.0) {
                        ...
                        if (isStepUpCandidate) {
                            ...
                            if (totalObstacleHeight <= stepHeight + 0.01) {
                                ...
                                if (mtvLen > 0.1) {
                                    val po =
                                        (mtvLen - 0.1) * 0.5; thisCorrectionX += dirX * po; thisCorrectionZ += dirZ * po
                                }
                            }
                    ...
                    if (isVertical && mtvLen > deepestVertMag) {
                        deepestVertMag = mtvLen; bestVertX = thisCorrectionX; bestVertY = thisCorrectionY; bestVertZ =
                            thisCorrectionZ
                    } else if (!isVertical && mtvLen > deepestHorizMag) {
                        deepestHorizMag = mtvLen; bestHorizX = thisCorrectionX; bestHorizY =
                            thisCorrectionY; bestHorizZ = thisCorrectionZ
                    }
                }
            }
