...
object VehicleVecUtils {
    ...
    fun setPassengerPitch(
        ...
        if (passengerRot == 180f) {
            val min = minPitch + vehicle.xRot
            val max = maxPitch + vehicle.xRot

            val f = Mth.wrapDegrees(entity.xRot)
            val f1 = Mth.clamp(f, min, max)
            entity.xRotO += f1 - f
            ...
            val min = -maxPitch - r * vehicle.xRot - r2 * vehicle.roll
            val max = -minPitch - r * vehicle.xRot - r2 * vehicle.roll
    ...
    fun setPassengerYaw(vehicle: VehicleEntity, entity: Entity, minYaw: Float, maxYaw: Float, passengerRot: Float) {
        if (passengerRot == 180f) {
            val f2 = Mth.wrapDegrees(entity.yRot - vehicle.yRot + passengerRot)
            val f3 = Mth.clamp(f2, minYaw, maxYaw)
            entity.yRotO += f3 - f2
            entity.yRot = entity.yRot + f3 - f2
        } else {
            val f2 = Mth.wrapDegrees(entity.yRot - vehicle.yRot)
            val f3 = Mth.clamp(f2, passengerRot + minYaw, passengerRot + maxYaw)
            entity.yRotO += f3 - f2
            entity.yRot = entity.yRot + f3 - f2
        }
        entity.setYBodyRot(vehicle.yRot + passengerRot)
    }

    fun setPassengerPitchOnTurret(
        vehicle: VehicleEntity,
        entity: Entity,
        turretMinPitch: Float,
        turretMaxPitch: Float
    ) {
        val a = vehicle.getTurretYaw(1f)
        val r = (Mth.abs(a) - 90f) / 90f
        ...
        val min = -turretMaxPitch - r * vehicle.xRot - r2 * vehicle.roll
        val max = -turretMinPitch - r * vehicle.xRot - r2 * vehicle.roll
    ...
    fun setPassengerYawOnTurret(
        ...
        val f2: Float
        if (passengerRot != 180f) {
            f2 = Mth.wrapDegrees(entity.yRot - vehicle.yRot)
            val f3 = Mth.clamp(f2, passengerRot + minYaw, passengerRot + maxYaw)
            entity.yRotO += f3 - f2
            entity.yRot = entity.yRot + f3 - f2
        } else {
            f2 = Mth.wrapDegrees(entity.yRot - vehicle.yRot + 180)
            val f3 = Mth.clamp(f2, minYaw, maxYaw)
            entity.yRotO += f3 - f2
            entity.yRot = entity.yRot + f3 - f2
        }
        ...
        if (rotateWithTurret) {
            entity.setYBodyRot(vehicle.getBarrelYRot(1f) + passengerRot)
        }

        clampZoomYaw(vehicle, entity)
    }

    fun clampZoomYaw(vehicle: VehicleEntity, entity: Entity) {
        ...
        if (vehicle.turretControllerIndex == vehicle.getSeatIndex(entity)) {
            val f2 = Mth.wrapDegrees(entity.yRot - vehicle.getBarrelYRot(1f))
            val f3 = Mth.clamp(f2, -20f, 20f)
            entity.yRotO += f3 - f2
            entity.yRot = entity.yRot + f3 - f2
        }
    }
    ...
    fun getBarrelTransform(vehicle: VehicleEntity, partialTicks: Float): Matrix4d {
        ...
        val xV = Mth.lerp(partialTicks, vehicle.xRotO, vehicle.xRot)
