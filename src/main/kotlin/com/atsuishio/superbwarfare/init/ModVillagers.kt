...
object ModVillagers {
    ...
    val POI_TYPES: DeferredRegister<PoiType> =
        DeferredRegister.create(ForgeRegistries.POI_TYPES, com.atsuishio.superbwarfare.Mod.MODID)

    @JvmField
    val VILLAGER_PROFESSIONS: DeferredRegister<VillagerProfession> =
        DeferredRegister.create(ForgeRegistries.VILLAGER_PROFESSIONS, com.atsuishio.superbwarfare.Mod.MODID)

    @JvmField
    val ARMORY_POI: RegistryObject<PoiType> = POI_TYPES.register("armory") {
        PoiType(
            ImmutableSet.copyOf(
                ModBlocks.REFORGING_TABLE.get().getStateDefinition().getPossibleStates()
            ), 1, 1
        )
    }

    @JvmField
    val ARMORY: RegistryObject<VillagerProfession> = VILLAGER_PROFESSIONS.register("armory") {
        VillagerProfession(
            "armory",
            { it.get() === ARMORY_POI.get() },
            { it.get() === ARMORY_POI.get() },
            ImmutableSet.of(),
            ImmutableSet.of(),
            null
    ...
    fun addWandererTrade(event: WandererTradesEvent) {
        ...
        rareTrades.add(
            BasicItemListing(
                ItemStack(Items.EMERALD, 16),
                SmallContainerBlockItem.createInstance(com.atsuishio.superbwarfare.Mod.loc("containers/blueprints")),
                10,
                0,
                0.05f
            )
        ...
        rareTrades.add(
            BasicItemListing(
                ItemStack(Items.EMERALD, 10),
                SmallContainerBlockItem.createInstance(com.atsuishio.superbwarfare.Mod.loc("containers/common")),
                10,
                0,
                0.05f
            )
